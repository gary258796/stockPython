...
def store_to_mysql(data):
    ...
    sql = "INSERT INTO ticker_list (stock_code) VALUES (%s)"
    for data in data:
...
def updateHistoryandStore2Drive(stockCodeList, driveName, period, start, end, interval, bool_action):
    ...
    for i in range(len(stockCodeList)):
        ...
        for retry in range(10):  # at most retry 10 times
            try:  # try get history and store to csv in local path
                # new data
                newHistory = getStockHistorybyYahoo(stockCodeList[i], period, start, end, interval, bool_action, bool_proxy)
...
def mainnnn():
    getAll = True
    upDate = False
    getAllorUpdate()  # get history all data by 1m, 2m, 1hr, 1day
...
def main():
    ...
    if do:
        ...
        getStockHistorybyYahooandStore2Drive(tw_stock_list[1:2], "onedrive", None, start, end, "2m", False)
